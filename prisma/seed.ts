import { PrismaClient, RoleType, ProjectStatus } from '@prisma/client'
...
async function main() {
  ...
  const projects = await Promise.all([
    prisma.project.create({
      data: {
        ...
        areaId: areas[0].id, // Agrotecnología
        categoryId: categories[2].id, // PMV
        institutionId: institutions[0].id, // Universidad Central
    ...
    prisma.project.create({
      data: {
        ...
        areaId: areas[2].id, // Economía circular
        categoryId: categories[3].id, // Emprendimiento en Ejecución
        institutionId: institutions[1].id, // Instituto Tecnológico Nacional
    ...
    prisma.project.create({
      data: {
        ...
        areaId: areas[3].id, // Salud
        categoryId: categories[2].id, // PMV
        institutionId: institutions[2].id, // Universidad de Innovación
    ...
    prisma.project.create({
      data: {
        ...
        areaId: areas[5].id, // Turismo sostenible
        categoryId: categories[1].id, // Poster
    ...
    prisma.project.create({
      data: {
        ...
        areaId: areas[6].id, // Innovación Abierta
        categoryId: categories[2].id, // PMV
    ...
    prisma.project.create({
      data: {
        ...
        areaId: areas[4].id, // Innovación Digital
        categoryId: categories[0].id, // Emprendimiento Escolar
        institutionId: institutions[0].id,
        ownerId: participant1.id,
      },
    }),
  ...
  await Promise.all([
    // Project 0 (AgroSense - SUBMITTED) - both evaluators assigned
    ...
    // Project 1 (BioPack - APPROVED) - both evaluators assigned
    prisma.projectEvaluator.create({
    ...
    // Project 2 (SaludConecta - FINALIST) - both evaluators assigned
    prisma.projectEvaluator.create({
    ...
    // Project 5 (BioFarma - WINNER) - both evaluators assigned
    prisma.projectEvaluator.create({
    ...
    // Project 4 (FinVerde - SUBMITTED) - evaluator1 assigned
    prisma.projectEvaluator.create({
      data: { projectId: projects[4].id, evaluatorId: evaluator1.id },
    }),
    // Project 7 (CirculaEC - APPROVED) - evaluator2 assigned
    prisma.projectEvaluator.create({
  ...
  // Evaluation for BioPack (project 1) by evaluator1 - completed
    ...
    data: [
      {
        ...
        score: 17,
        maxScore: 20,
        observation: 'Solución creativa con buen enfoque diferenciador.',
      },
      {
        evaluationId: eval1.id,
        ...
        score: 13,
        maxScore: 15,
        observation: 'Modelo de negocio claro con proyecciones realistas.',
      },
      {
        evaluationId: eval1.id,
        criteriaId: criteria[2].id,
        score: 14,
        maxScore: 15,
        observation: 'Excelente impacto ambiental demostrado.',
      },
      {
        evaluationId: eval1.id,
        criteriaId: criteria[3].id,
        score: 16,
        maxScore: 20,
        observation: 'Buena presentación, podría mejorar el manejo del tiempo.',
      },
      {
        evaluationId: eval1.id,
        criteriaId: criteria[4].id,
        score: 8,
        maxScore: 10,
        observation: 'Potencial de expansión a otras regiones agrícolas.',
      },
      {
        evaluationId: eval1.id,
        criteriaId: criteria[5].id,
        score: 17,
        maxScore: 20,
        observation: 'Documentación completa y bien presentada.',
      },
    ],
  })

  // Evaluation for BioPack (project 1) by evaluator2 - completed
    ...
    data: [
      {
        ...
        score: 16,
        maxScore: 20,
        observation: 'Innovación moderada, existen alternativas similares.',
      },
      {
        evaluationId: eval2.id,
        criteriaId: criteria[1].id,
        score: 12,
        maxScore: 15,
        observation: 'Viabilidad comercial demostrada con cartas de intención.',
      },
      {
        evaluationId: eval2.id,
        criteriaId: criteria[2].id,
        score: 15,
        maxScore: 15,
        observation: 'Impacto ambiental excepcional y medible.',
      },
      {
        evaluationId: eval2.id,
        criteriaId: criteria[3].id,
        score: 14,
        maxScore: 20,
        observation: 'Presentación correcta pero falta más pasión en el pitch.',
      },
      {
        evaluationId: eval2.id,
        criteriaId: criteria[4].id,
        score: 7,
        maxScore: 10,
        observation: 'Escalabilidad limitada por disponibilidad de materia prima.',
      },
      {
        evaluationId: eval2.id,
        criteriaId: criteria[5].id,
        score: 16,
        maxScore: 20,
        observation: 'Entregables completos y bien organizados.',
      },
    ],
  })

  // Evaluation for SaludConecta (project 2) by evaluator1 - completed
    ...
    data: [
      {
        ...
        score: 19,
        maxScore: 20,
        observation: 'Innovación destacada con IA para triaje automatizado.',
      },
      {
        evaluationId: eval3.id,
        criteriaId: criteria[1].id,
        score: 14,
        maxScore: 15,
        observation: 'Modelo de negocio sostenible con múltiples fuentes de ingreso.',
      },
      {
        evaluationId: eval3.id,
        criteriaId: criteria[2].id,
        score: 15,
        maxScore: 15,
        observation: 'Impacto social transformador en comunidades vulnerables.',
      },
      {
        evaluationId: eval3.id,
        criteriaId: criteria[3].id,
        score: 17,
        maxScore: 20,
        observation: 'Pitch convincente con datos duros que respaldan.',
      },
      {
        evaluationId: eval3.id,
        criteriaId: criteria[4].id,
        score: 9,
        maxScore: 10,
        observation: 'Alto potencial de replicabilidad en toda la región.',
      },
      {
        evaluationId: eval3.id,
        criteriaId: criteria[5].id,
        score: 18,
        maxScore: 20,
        observation: 'Entregables de calidad profesional.',
      },
    ],
  })

  // Evaluation for SaludConecta (project 2) by evaluator2 - completed
  ...
  await prisma.evaluationScore.createMany({
    data: [
      {
        evaluationId: eval4.id,
        criteriaId: criteria[0].id,
        score: 18,
        maxScore: 20,
        observation: 'Buena innovación tecnológica, clara diferenciación.',
      },
      {
        evaluationId: eval4.id,
        criteriaId: criteria[1].id,
        score: 13,
        maxScore: 15,
        observation: 'Viabilidad comprobada con datos de la prueba piloto.',
      },
      {
        evaluationId: eval4.id,
        criteriaId: criteria[2].id,
        score: 14,
        maxScore: 15,
        observation: 'Impacto social significativo y medible.',
      },
      {
        evaluationId: eval4.id,
        criteriaId: criteria[3].id,
        score: 16,
        maxScore: 20,
        observation: 'Buena presentación, datos claros.',
      },
      {
        evaluationId: eval4.id,
        criteriaId: criteria[4].id,
        score: 8,
        maxScore: 10,
        observation: 'Escalable a otras zonas rurales del país.',
      },
      {
        evaluationId: eval4.id,
        criteriaId: criteria[5].id,
        score: 19,
        maxScore: 20,
        observation: 'Documentación exhaustiva y profesional.',
      },
    ],
  })

  // Evaluation for BioFarma (project 5) by evaluator1 - completed (WINNER)
    ...
    data: [
      {
        ...
        score: 20,
        maxScore: 20,
        observation: 'Innovación excepcional al combinar saberes ancestrales con ciencia moderna.',
      },
      {
        evaluationId: eval5.id,
        criteriaId: criteria[1].id,
        score: 14,
        maxScore: 15,
        observation: 'Mercado farmacéutico claro con alto potencial.',
      },
      {
        evaluationId: eval5.id,
        criteriaId: criteria[2].id,
        score: 15,
        maxScore: 15,
        observation: 'Impacto social y cultural extraordinario.',
      },
      {
        evaluationId: eval5.id,
        criteriaId: criteria[3].id,
        score: 18,
        maxScore: 20,
        observation: 'Pitch inspirador con narrativa impactante.',
      },
      {
        evaluationId: eval5.id,
        criteriaId: criteria[4].id,
        score: 9,
        maxScore: 10,
        observation: 'Potencial de expansión a toda la región andina.',
      },
      {
        evaluationId: eval5.id,
        criteriaId: criteria[5].id,
        score: 20,
        maxScore: 20,
        observation: 'Entregables impecables, nivel de publicación científica.',
      },
    ],
  })

  // Evaluation for BioFarma (project 5) by evaluator2 - completed (WINNER)
    ...
    data: [
      {
        ...
        score: 19,
        maxScore: 20,
        observation: 'Innovación destacable en el sector farmacéutico.',
      },
      {
        evaluationId: eval6.id,
        criteriaId: criteria[1].id,
        score: 14,
        maxScore: 15,
        observation: 'Excelente proyección comercial.',
      },
      {
        evaluationId: eval6.id,
        criteriaId: criteria[2].id,
        score: 15,
        maxScore: 15,
        observation: 'Impacto cultural y sanitario excepcional.',
      },
      {
        evaluationId: eval6.id,
        criteriaId: criteria[3].id,
        score: 17,
        maxScore: 20,
        observation: 'Presentación sólida y bien estructurada.',
      },
      {
        evaluationId: eval6.id,
        criteriaId: criteria[4].id,
        score: 9,
        maxScore: 10,
        observation: 'Alto potencial de escalamiento internacional.',
      },
      {
        evaluationId: eval6.id,
        criteriaId: criteria[5].id,
        score: 20,
        maxScore: 20,
        observation: 'Documentación de calidad excepcional.',
      },
    ],
  })

  // Draft evaluation for AgroSense (project 0) by evaluator1
